...
public final class FAMMBlocks{
	...
	public static Block get(String id){
		...
		for(Block block : blocks){
			if(((IBlock)block).getName().equals(id)){
				result = block;
				break;
			}
		}
	...
	public static void initlizeDefault(){
		for(String s : blocknames){
			Block block = new FAMMBLK(s);
			blocks.add(block);
		}
    }
	
	public static void initlizeHalfBlocks(){
		for(String s : blocknames){
			s = s + "_hb";
			Block block = new FAMMBLKH(s);
			blocks.add(block);
		}
    }
}