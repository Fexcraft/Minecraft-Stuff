...
public class CTower extends BlockContainer implements IPaintableBlock{
	...
	public static class IB extends FIB {
		...
		public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ){
	        ...
	        if (stack.stackSize != 0 && playerIn.canPlayerEdit(pos, facing, stack) && worldIn.canBlockBePlaced(this.block, pos, false, facing, (Entity)null, stack)){
	            ...
	            IBlockState iblockstate1 = BGE.ct.getDefaultState().withProperty(CTower.COLOR, EnumDyeColor.byMetadata(i));
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ){
        if(!world.isRemote){
    		...
    		if(world.canBlockBePlaced(BGE.ct, p, false, side, (Entity)null, null)){
    			world.setBlockState(pos, Blocks.AIR.getDefaultState());
    			world.setBlockState(p, BGE.ct.getDefaultState().withProperty(COLOR, (EnumDyeColor)state.getValue(COLOR)));
    			return true;
    		}
        }
		return false;
