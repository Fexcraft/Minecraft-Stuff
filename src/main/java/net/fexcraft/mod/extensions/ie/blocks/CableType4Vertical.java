...
public class CableType4Vertical extends CableType4{

	public CableType4Vertical() {
		super();
	}

	@Override
	public String getName(){
		return "cabletype4_vertical";
	}

	@Override
	public TileEntity createNewTileEntity(World worldIn, int meta){
		return new CableType4VerticalEntity();
	}
	...
	public boolean onBlockActivated(World w, BlockPos pos, IBlockState state, EntityPlayer p, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ){
		if(!w.isRemote && hand == EnumHand.MAIN_HAND){
			if(p.getHeldItemMainhand() == null){
				...
				CableType4VerticalEntity te = (CableType4VerticalEntity)w.getTileEntity(pos);
			...
			if(p.getHeldItemMainhand() != null && p.getHeldItemMainhand().getItem() == Items.GOLD_INGOT){
				int i = ((CableType4VerticalEntity)w.getTileEntity(pos)).getIO().get(p.getHorizontalFacing().getOpposite()).cthickness;
				Print.spam(1, i + "");
				switch(i){
					case 2:
						i = 4;
			...
			if(p.getHeldItemMainhand() != null && p.getHeldItemMainhand().getItem() == Items.COAL){
				int i = ((CableType4VerticalEntity)w.getTileEntity(pos)).getIO().get(p.getHorizontalFacing().getOpposite()).cthickness;
				switch(i){
					case 2:
						i = 4;
				...
				boolean b = BoolUtil.reverse(((CableType4VerticalEntity)w.getTileEntity(pos)).getIO().vertByFacing(p.getHorizontalFacing().getOpposite()));
				((CableType4VerticalEntity)w.getTileEntity(pos)).getIO().vertByFacingSet(p.getHorizontalFacing().getOpposite(), b);
				((ICableTileEntity)w.getTileEntity(pos)).detectAndSendChanges(side);
			}
			if(p.getHeldItemMainhand() != null && p.getHeldItemMainhand().getItem() == Items.DIAMOND){
				((CableType4VerticalEntity)w.getTileEntity(pos)).resetFacing(p.getHorizontalFacing().getOpposite());
				((ICableTileEntity)w.getTileEntity(pos)).detectAndSendChanges(side);
			}
			return true;
		}
